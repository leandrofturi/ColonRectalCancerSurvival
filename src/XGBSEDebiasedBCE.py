...
OUTPUT_DIR = "data/resultados/XGBSEDebiasedBCE"
os.makedirs(OUTPUT_DIR, exist_ok=True)

class BaseEstimatorWrapper(BaseEstimator, RegressorMixin):
    """
    Wrapper sklearn-friendly para XGBSEDebiasedBCE:
      - suporte a grid de xgb_params__*
      - suporte a grid de lr_params__*
    ...
    def __init__(self, time_bins=None, **kwargs):
        ...
        for k, v in kwargs.items():
            ...
            elif k.startswith("lr_params__"):
                self.lr_params[k.replace("lr_params__", "")] = v
            else:
                self.est_params[k] = v
        self.model: Optional[XGBSEDebiasedBCE] = None

    def fit(self, X, y):
        self.model = XGBSEDebiasedBCE(xgb_params=self.xgb_params, lr_params=self.lr_params, **self.est_params)
    ...
    def get_params(self, deep=True):
        ...
        params.update({f"lr_params__{k}": v for k, v in self.lr_params.items()})
...
if __name__ == "__main__":
    ...
    param_grid = {
        ...
        "lr_params__penalty": ["l1", "l2", "elasticnet"],
        "lr_params__C": [0.01, 1, 10], 
    }
