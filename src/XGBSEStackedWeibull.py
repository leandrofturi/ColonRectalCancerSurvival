...
OUTPUT_DIR = "data/resultados/XGBSEStackedWeibull"
os.makedirs(OUTPUT_DIR, exist_ok=True)

class BaseEstimatorWrapper(BaseEstimator, RegressorMixin):
    """
    Wrapper sklearn-friendly para XGBSEStackedWeibull:
      - suporte a grid de xgb_params__*
      - suporte a grid de weibull_params__*
    ...
    def __init__(self, time_bins=None, **kwargs):
        ...
        self.weibull_params: Dict[str, Any] = {}
        ...
        for k, v in kwargs.items():
            ...
            elif k.startswith("weibull_params__"):
                self.weibull_params[k.replace("weibull_params__", "")] = v
            else:
                self.est_params[k] = v
        self.model: Optional[XGBSEStackedWeibull] = None

    def fit(self, X, y):
        self.model = XGBSEStackedWeibull(xgb_params=self.xgb_params, weibull_params=self.weibull_params, **self.est_params)
    ...
    def get_params(self, deep=True):
        ...
        params.update({f"weibull_params__{k}": v for k, v in self.weibull_params.items()})
...
if __name__ == "__main__":
    ...
    param_grid = {
        ...
        "weibull_params__fit_intercept": [True, False],
        "weibull_params__penalizer": [0.01, 1, 10], 
    }
