...
# ====================================
# Modificado por Matheus Becali Rocha
# 08/14/25
# Email: matheusbecali@gmail.com
# ====================================
class surv_likelihood:
  """
  Classe de perda customizada para o modelo de sobrevivência.
  """
  ...
  def __call__(self, y_pred, y_true):
    ...
    uncens = 1. - y_true[:, self.n_intervals:2*self.n_intervals] * y_pred
    concatenated = torch.cat((cens_uncens, uncens), dim=-1)
    
    # Usar o dtype do tensor para compatibilidade com diferentes precisões (float, half, etc.)
    clipped = torch.clamp(concatenated, min=torch.finfo(concatenated.dtype).eps)
